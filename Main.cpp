...
//-----------------------------------------------------------------
//CyberBio 
//На канале Youtube "Cyber биолог" https://www.youtube.com/channel/UC8VRsAJRpwatj3tGae2mYMg
//есть видео с работой программы
// 
//Качество кода, мягко говоря, не идеально, но это хобби-проект, изначально публиковать не планировал.
//Программа выросла из одного файла, отсюда и код в хэдэрах, всё на скорую руку.
// 
// 
...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance,	_In_opt_ HINSTANCE hPrevInstance,	_In_ LPWSTR    lpCmdLine,	_In_ int       nCmdShow)
{
	...
	for (;;)
	{
		...
		if (io.MouseDown[0])
		{
			if (field->IsInBoundsScreenCoords(mouseX, mouseY))
			{
		...
		ImGui::Begin("Selection", NULL, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoCollapse);
		{
			if (selection)
			{
				if (field->ValidateObjectExistance(selectedObject))
				{
					...
					//ImGui::Text("{%i, %i, %i, %i}", m[0], m[1], m[2]);
